...
Menu::Menu(SDL_Window* window,SDL_Renderer* renderer){
	...
	rect.x = this->ancho / 4;
	this->textInputNombre = new TextInput(renderer, rect,"Nombre: ",50,0,255,0);
	...
	rect.x = this->ancho / 4;
	this->textInputIP = new TextInput(renderer, rect, "IP: ", 50,0,255,0);
	this->listaDibujables->push_back(dib);
}
...
void Menu::dibujar(){
	...
	if (!this->menuCliente) {
		...
		//this->textInput->dibujar(this->renderer, 0, 0, 1, this->ancho, this->alto);
	} else {
...
int Menu::leerEvento(){
	if (SDL_PollEvent(this->evento) != 0) {
		...
		if (evento->type == SDL_KEYDOWN) {
			...
			case SDLK_RETURN: 	
				if (this->textInputNombre->getTexto().size() != 0 && this->textInputIP->getTexto().size() != 0)
					return nameMenu::CLIENTE; break;
			case SDLK_TAB: 
